...
export const applyToJob = async (req, res) => {
  ...
  try {
    ...
    // Application is saved — don't fail the request
  }
...
export const searchApplicants = async (req, res) => {
  ...
  const pipeline = [
    ...
    {
      $lookup: {
        from: 'users', localField: 'candidate',
        foreignField: '_id', as: 'candidateData'
      }
    },
    { $unwind: '$candidateData' },
    {
      $lookup: {
        from: 'jobs', localField: 'job',
        foreignField: '_id', as: 'jobData'
      }
    },
    { $unwind: '$jobData' },
    {
      $lookup: {
        from: 'resumes', localField: 'resume',
        foreignField: '_id', as: 'resumeData'
      }
    },
  ...
  pipeline.push({
    $project: {
      _id: 1, status: 1, aiScore: 1, aiReasoning: 1,
      aiMissingSkills: 1, aiInterviewQuestions: 1,
      githubInsights: 1, embeddingScore: 1, createdAt: 1, xai: 1,
      candidate: {
      ...
      job:    { _id: '$jobData._id', title: '$jobData.title', company: '$jobData.company' },
  ...
  const countResult = await Application.aggregate([...pipeline, { $count: 'total' }])
    ...
    { $match: appMatch },
    {
      $facet: {
        ...
        scoreRanges: [
          {
            $bucket: {
              groupBy: '$aiScore', boundaries: [0, 25, 50, 75, 101],
              default: 'unscored', output: { count: { $sum: 1 } }
            }
          }
        ],
        topMissingSkills: [
