...
screeningQueue.process(async (job) => {
  ...
  try {
    ...
    if (!resume.parsedText || resume.parsedText.length < 20) {
      throw new Error('Resume has no parsed text — PDF may be image-based or corrupted')
    }
    ...
    const scoreResult = githubSummary
      ? await scoreResumeWithGitHub(
          resume.parsedText, jobDoc.description,
          jobDoc.skillsRequired, githubSummary
        )
      : await scoreResume(
          resume.parsedText, jobDoc.description,
    ...
    console.log(`📊 Final: ${finalScore} (xai:${xaiScore} std:${scoreResult.score} emb:${embeddingScore})`)
...
screeningQueue.on('failed',    (job, err) => console.error(`Queue job ${job.id} failed: ${err.message}`))
screeningQueue.on('completed', (job, res) => console.log(`Queue job ${job.id} done:`, res))
